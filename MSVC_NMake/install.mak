...
install: all
	...
	@for %h in ($(files_built_h)) do @if exist vs$(PDBVER)\$(CFG)\$(PLAT)\pangomm\%h copy vs$(PDBVER)\$(CFG)\$(PLAT)\pangomm\%h "$(PREFIX)\include\pangomm-$(PANGOMM_MAJOR_VERSION).$(PANGOMM_MINOR_VERSION)\pangomm\%h"
	...
	@for %h in ($(PANGOMM_BUILD_PRIVATE_HEADERS)) do @if exist vs$(PDBVER)\$(CFG)\$(PLAT)\pangomm\private\%h copy vs$(PDBVER)\$(CFG)\$(PLAT)\pangomm\private\%h "$(PREFIX)\include\pangomm-$(PANGOMM_MAJOR_VERSION).$(PANGOMM_MINOR_VERSION)\pangomm\private\%h"
